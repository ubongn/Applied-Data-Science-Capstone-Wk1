...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, names= 'class', title='Total Success Launches for '+ entered_site)
...
def get_scatter_plot(dropdown, slider):
    if dropdown == 'ALL':
        ...
        print(filtered_df)
        filtered_df1 = filtered_df[(filtered_df['Payload Mass (kg)'] >= slider[0]) & (filtered_df['Payload Mass (kg)'] <= slider[1])]
        fig = px.scatter(filtered_df1, x='Payload Mass (kg)', y='class',
                         color='Booster Version Category',
                         title='Correlation between Payload and Success for '+ dropdown)
        return fig
